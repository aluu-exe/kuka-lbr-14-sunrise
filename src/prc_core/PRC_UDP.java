...
public class PRC_UDP {
	...
	public void CORE_UDP(LBR robot, Controller kuka_Sunrise_Cabinet_1, SpatialObject tool, String tcpname, ObjectFrame baseFrame, boolean enablelogging, IApplicationData AppData, AbstractIOGroup ioGroup, String ip, int port) throws SocketException, UnknownHostException {
		...
		while (true) {
			...
			for (PRC_CommandData cmd : udpcmds) {
				...
				if (cmd.prccmdType.equals(PRC_Enums.ANOUT)){
					...
					if (enablelogging){
						// logger.info(cmd.axisMove.ToString());
					}
					...
					if (enablelogging){
						// logger.info(cmd.kmrMove.ToString());
					}
					...
					if (enablelogging){
						// logger.info(cmd.cirMove.ToString());
					}
					...
					if (enablelogging){
						// logger.info(cmd.linMove.ToString());
					}
					...
					if (enablelogging){
						// logger.info(cmd.linCompMove.ToString());
					}
					...
					if (enablelogging){
						// logger.info(cmd.ptpMove.ToString());
					}
					...
					if (enablelogging){
						// logger.info(cmd.ptpCompMove.ToString());
					}
					...
					if (enablelogging){
						// logger.info(cmd.motionBatch.ToString());
					}
					...
					if (enablelogging){
						// logger.info(cmd.splMove.ToString());
					}
				} 
